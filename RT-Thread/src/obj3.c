...
#define CAN_ECN_GEN_OBJ_HANDLE(objName) (CanEcnHndGeneral)(&objName)
#define CAN_ECN_GENERAL_CONSTRUCT(objName, canEcnDataTbl, canEcnDataTblSize, canEcnModemToRh850Tbl, \
								  canEcnModeToRh850TblSize)
			static EcnCanDataPacketType objName##sigEcnDataPacket = {0};
			const EcnCanGeneralType objName = {&objName##sigEcnDataPacket, canEcnDataTbl, \
											   canEcnDataTblSize, canEcnModemToRh850Tbl, canEcnModeToRh850TblSize}
											   
typedef struct EcnCanGeneralObj *CanEcnHndGeneral;
typedef struct EcnCanGeneralObj {EcnCanDataPacketType *sigEcnDataPacket; \
								 EcnCanSigDataTblType * const canEcnDataTbl; \
								 const uint8_t canEcnDataTblSize; \
								 const EcnCanSigModeToRh850TblType * const canEcnModemToRh850Tbl; \
								 const uint8_t canEcnModeToRh850TblSize; \
							} EcnCanGeneralType;
							
CAN_ECN_GENERAL_CONSTRUCT(g_canEcnGeneral, g_canEcnDataToModeTbl, (sizeof(g_canEcnDataToModeTbl) / sizeof(EcnCanSigDataTblType)), \
						  g_canEcnDataModeTo850Tbl, (sizoef(g_canEcnDataModeTo850Tbl) / sizeof(EcnCanSigModeToRh850TblType)));
						  
void CanSendEcnSigToModemGandle(void)
{
	CanEcnHndGeneral this = CAN_ECN_GEN_OBJ_HANDLE(g_canEcnGeneral);
}
